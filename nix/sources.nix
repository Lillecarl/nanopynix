...
let
  ...
  # working copy or a pinned store path, this is `<umbrella>/nanopynix`.
  ...
  # `refs/umbrella/nanopynix/*`. `umbrella mark` publishes them.
  #
  ...
  # `github:Lillecarl/nanopynix`, a release tarball, or a pull request from a
  ...
  umbrellaRef =
    if umbrellaRev != "" then
      ...
      throw ''
        nix/sources.nix: no umbrella to build nanopynix against.
        ...
        nearest landed ancestor of HEAD:
          ...
          ci/walkback.sh https://github.com/nixidae/nixidae nanopynix
...
in
import wire { overrides.nanopynix = ../.; }
