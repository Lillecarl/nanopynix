...
let
  ts = toString builtins.currentTime;
  ...
  mkDrv = { name, deps ? [], sleepSecs ? 1, text ? "" }:
  ...
  mkParallel = { leaves, sleepSecs, parId ? "" }:
    let
      prefix = if parId == "" then "" else "${parId}-";
      parLeaves = builtins.genList (i:
        mkDrv {
  ...
  a0 = mkDrv { name = "leaf-a0"; sleepSecs = 2; text = "leaf a0"; };
  a1 = mkDrv { name = "leaf-a1"; sleepSecs = 1; text = "leaf a1"; };
  a2 = mkDrv { name = "leaf-a2"; sleepSecs = 3; text = "leaf a2"; };
  a3 = mkDrv { name = "leaf-a3"; sleepSecs = 1; text = "leaf a3"; };
  ...
  b0 = mkDrv { name = "mid-b0"; deps = [ a0 ];     sleepSecs = 1; text = "depends a0"; };
  b1 = mkDrv { name = "mid-b1"; deps = [ a0 a1 ];  sleepSecs = 1; text = "depends a0+a1"; };
  b2 = mkDrv { name = "mid-b2"; deps = [ a1 a2 ];  sleepSecs = 2; text = "depends a1+a2"; };
  b3 = mkDrv { name = "mid-b3"; deps = [ a2 ];     sleepSecs = 1; text = "depends a2"; };
  b4 = mkDrv { name = "mid-b4"; deps = [ a3 ];     sleepSecs = 1; text = "depends a3"; };
  b5 = mkDrv { name = "mid-b5"; deps = [ a2 a3 ];  sleepSecs = 1; text = "depends a2+a3"; };

  # ── Layer 2: 3 top nodes (fan-in) ──────────────────────────────
  c0 = mkDrv { name = "top-c0"; deps = [ b0 b1 b2 ]; sleepSecs = 1; text = "fan-in b0+b1+b2"; };
  c1 = mkDrv { name = "top-c1"; deps = [ b3 b4 ];    sleepSecs = 1; text = "fan-in b3+b4"; };
  c2 = mkDrv { name = "top-c2"; deps = [ b4 b5 ];    sleepSecs = 1; text = "fan-in b4+b5"; };

  # ── Root: depends on all tops ───────────────────────────────────
  root = mkDrv { name = "root"; deps = [ c0 c1 c2 ]; sleepSecs = 1; text = "root"; };

  # ── Env-configured parallel (for test_parallel.py compat) ───────
  parCount = let v = builtins.getEnv "PYNIXD_PAR_COUNT"; in
    if v == "" then 100 else builtins.fromJSON v;
  parId = let v = builtins.getEnv "PYNIXD_PAR_ID"; in
    if v == "" then "" else v;
  parSleep = let v = builtins.getEnv "PYNIXD_PAR_SLEEP"; in
