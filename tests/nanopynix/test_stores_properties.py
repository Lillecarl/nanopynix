...
#: One piece of the path part of an authority. Every supported Nix leaves this
#: half as the URI wrote it, so the escaping survives and the same alphabet
#: that the host half uses works here. Nix 2.31 decoded it while parsing and
#: needed a narrower alphabet; that version is gone.
_PATH_PIECE = _HOST_PIECE
...
def test_the_host_part_of_an_authority_is_escaped_on_every_version(host: str) -> None:
    ...
    an authority looked alike, and an early reading put the whole authority
    under one rule and gave up on carrying a ``#`` at all. Nix reads the two
    halves with different code. Measured over the whole of ASCII.
    """
