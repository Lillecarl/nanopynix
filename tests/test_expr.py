...
class TestEvalAttrs:
    def test_eval_simple_attrs(self, eval_state):
    ...
    def test_attr_get(self, eval_state):
        v = eval_state.eval_string("{ x = 42; }")
        x = v.attr_get("x")
        assert x.as_int() == 42

    def test_attr_get_missing_raises(self, eval_state):
        import pytest
    ...
    def test_attr_get_on_non_attrs_raises(self, eval_state):
        import pytest
...
class TestEvalList:
    def test_eval_list(self, eval_state):
        v = eval_state.eval_string("[1 2 3]")
        assert v.type() == "list"
        assert v.list_length() == 3

    def test_list_get(self, eval_state):
...
class TestValueTypes:
    def test_is_methods_int(self, eval_state):
        v = eval_state.eval_string("1")
        assert v.is_int()
        assert not v.is_float()
        assert not v.is_string()

    def test_is_methods_float(self, eval_state):
        v = eval_state.eval_string("1.5")
        assert v.is_float()
        assert not v.is_int()

    def test_is_methods_bool(self, eval_state):
        v = eval_state.eval_string("true")
        assert v.is_bool()
        assert not v.is_int()

    def test_is_methods_string(self, eval_state):
        v = eval_state.eval_string('"x"')
        assert v.is_string()
        assert not v.is_attrs()

    def test_is_methods_attrs(self, eval_state):
        v = eval_state.eval_string("{}")
        assert v.is_attrs()

    def test_is_methods_list(self, eval_state):
        v = eval_state.eval_string("[]")
        assert v.is_list()

    def test_is_methods_function(self, eval_state):
        v = eval_state.eval_string("x: x")
        assert v.is_function()

    def test_is_methods_null(self, eval_state):
        v = eval_state.eval_string("null")
        assert v.is_null()


class TestForce:
    def test_force(self, eval_state):
        v = eval_state.eval_string("1 + 2")
        v.force()  # Should not raise

    def test_force_deep(self, eval_state):
        v = eval_state.eval_string("rec { a = 1 + 2; b = [(3 + 4)]; }")
        v.force_deep()  # Should not raise


class TestEvalFile:
    def test_eval_file_simple(self, eval_state, tmp_path):
...
class TestAllocValue:
    def test_alloc_value(self, eval_state):
