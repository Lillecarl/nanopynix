...
async def test_eval_session_cleanup(tmp_path):
    ...
    async with Nix(max_workers=1) as nix:
        async with nix.eval() as session:
            root = await session.eval_file(str(nix_file))
            await root.force()
        # Session closed — worker is returned to pool for store calls
...
async def test_eval_thunk(tmp_path):
    ...
    async with Nix(max_workers=1) as nix:
        async with nix.eval() as session:
            ...
            assert root.type_name == "attrs"
            x = await root.attr("x")
            # Should be thunk if not forced
            # (actually, attr_get may or may not force it depending on implementation)
