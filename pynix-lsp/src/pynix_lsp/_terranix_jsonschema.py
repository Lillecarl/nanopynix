...
everything else (``pynix_lsp._jsonschema``) is format-agnostic and reusable for
...
object, so ``pynix_lsp._jsonschema.validate``/``walk``/``render`` can operate on
...
if TYPE_CHECKING or BEARTYPING:
    from pynix_lsp._terranix_schema import SchemaAttribute, SchemaBlock
